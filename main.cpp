...
int main(){
	...
	//initial tank screen
	for (int i=0 ; i<getMaxNumOFplayer()/*4*/ ; i++)
	{	
	...
	//initial wall screen
	for (int i=0 ; i<getMaxNumOFmap()/*4*/ ; i++)
	{
	...
	for (int i=0 ; i<numbersOFplayerINgame ; i++)
	{
		tank[i].set_health(5);
	...
	bool quit=false;
	SDL_Rect offset;
	int number_of_map_in_game=randomMap( getMaxNumOFmap() );
	while (quit==false)
	{
    ...
    //If a key was released
		...
		if(flagdown==true)
		{
		 ...
		 if (flagup==false&&flagdown==false)
		{
			tank[0].set_Xspeed(0);
			tank[0].set_Yspeed(0);
		}
		...
		SDL_BlitSurface(mapScreen[number_of_map_in_game],NULL,SCREEN,NULL);
